...
- shallow-clones the repo into `my-app/`
- generates a strong `SESSION_SECRET` into `.env`
- runs `npm ci` and applies migrations

Flags: `--branch <name>`, `--no-install`.
...
| `DB_PATH` | `express-inertia.db` | SQLite file path |
...
├── config/           # Env validation (Zod), page name registry
...
Add the page name to `src/config/pages.ts` to keep it type-safe.
...
| `concurrently` | Run multiple processes | Lets `npm run dev` run server + Vite watch in one terminal. |
