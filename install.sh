...
REPO_URL="${TBA_REPO:-https://github.com/alphaofficial/theboringarchitecture.git}"
REPO_API="${TBA_REPO_API:-https://api.github.com/repos/alphaofficial/theboringarchitecture}"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
The Boring Architecture installer
...
# --- gather settings --------------------------------------------------------
DEFAULT_NAME="${TARGET_DIR:-my-tba-app}"
DEFAULT_NAME=$(slugify "$DEFAULT_NAME")
[ -n "$DEFAULT_NAME" ] || DEFAULT_NAME="my-tba-app"
...
if [ "$QUICK" = "1" ]; then
  ...
  APP_DESC="An app built with The Boring Architecture"
  ...
  APP_DESC=$(prompt "Description" "An app built with The Boring Architecture")
...
APP_SLUG="$APP_SLUG" APP_DESC="$APP_DESC" APP_AUTHOR="$APP_AUTHOR" TBA_REF="$REF" TBA_REF_KIND="$REF_KIND" \
  node - <<'NODE'
const fs = require('fs');
...
pkg.license = 'MIT';
pkg.tba = { version: process.env.TBA_REF, kind: process.env.TBA_REF_KIND };
...
cat > .env <<EOF
# Generated by The Boring Architecture installer
...
if [ "$DO_GIT" = "1" ]; then
  ...
  if [ -n "$GIT_AUTHOR" ] && [ -n "$GIT_EMAIL" ]; then
    git commit -q -m "Initial commit from The Boring Architecture" || true
  else
    git -c user.name="tba" -c user.email="tba@local" commit -q -m "Initial commit from The Boring Architecture" || true
  fi
  ok "git initialized"
fi
