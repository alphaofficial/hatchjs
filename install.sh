#!/usr/bin/env bash
#
# Hatch installer — scaffolds a fullstack starter into a fresh directory.
#
...
if [ "$QUICK" = "1" ]; then
  ...
  APP_DESC=$(prompt "Description" "A Hatch app")
...
cat > .env <<EOF
# Generated by the Hatch installer
...
if [ "$DO_GIT" = "1" ]; then
  ...
  if [ -n "$GIT_AUTHOR" ] && [ -n "$GIT_EMAIL" ]; then
    git commit -q -m "Initial commit from Hatch" || true
  else
    git -c user.name="hatch" -c user.email="hatch@local" commit -q -m "Initial commit from Hatch" || true
  fi
  ok "git initialized"
fi
